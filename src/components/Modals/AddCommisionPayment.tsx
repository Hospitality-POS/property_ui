...
const calculateAmountPaid = (sale) => {
  ...
  if (sale?.saleData) {
    ...
    if (
      sale.saleData.payments &&
      Array.isArray(sale.saleData.payments) &&
      sale.saleData.payments.length > 0
    ) {
  ...
  useEffect(() => {
    if (sale && visible) {
      ...
      const commissionData = {
        ...
        paid:
          sale.commission?.payments?.reduce(
        ...
        <Col span={24}>
          <Card>
            ...
            <Row>
              <Col span={12}>
                <Text>
                  Property: <strong>{sale?.property?.name || 'N/A'}</strong>
                </Text>
              </Col>
              <Col span={12}>
                <Text>
                  Unit: <strong>{sale?.unit?.unitType || 'N/A'}</strong>
                </Text>
              </Col>
            </Row>
            <Row>
              ...
              <Col span={12}>
                <Text>
                  Agent: <strong>{sale?.salesAgent?.name || 'N/A'}</strong>
                </Text>
              </Col>
            </Row>
          </Card>
                      ...
                      validator: (_, value) => {
                        if (value > commission.accrued - commission.paid) {
                          return Promise.reject(
                            new Error(
                              'Amount exceeds available accrued commission',
                            ),
                          );
                        }
