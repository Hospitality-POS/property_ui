...
import { BankOutlined, UserOutlined, SafetyCertificateOutlined, DeleteOutlined } from '@ant-design/icons';
import { ProForm, ProFormText } from '@ant-design/pro-components';
import { useRequest } from '@umijs/max';
import { Card, Typography, Divider, message, Steps, Alert, Space, Input, Button } from 'antd';
import { requestOTP, verifyOTP, deleteAccount } from '@/services/auth.api';
...
const DeleteAccountPage = () => {
    ...
    // Request OTP
    const { run: runRequestOTP, loading: isRequestingOTP } = useRequest(requestOTP, {
        manual: true,
        onSuccess: () => {
        ...
        onError: (error: any) => {
            if (error.response?.status === 404) {
                message.error('Account not found. Please check your company code and email.');
            } else {
    ...
    // Delete Account
    const { run: runDeleteAccount, loading: isDeletingAccount } = useRequest(deleteAccount, {
        manual: true,
        onSuccess: () => {
            ...
            <ProFormText
                ...
                fieldProps={{
                    ...
                    onChange: (e) => {
                        ...
                        const newEvent = { ...e };
                        newEvent.target = { ...e.target, value: e.target.value.replace(/\s/g, '') };
                        // Use the new event
    ...
    const renderStep3 = () => (
        <div className="space-y-6 text-center">
            ...
            <div className="p-6 border border-red-300 rounded-lg bg-red-50">
                <Title level={4} className="text-red-600">Warning: Permanent Account Deletion</Title>
                <Paragraph>
